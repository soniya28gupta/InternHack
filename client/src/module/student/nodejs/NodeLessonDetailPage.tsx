...
import type { NodeProgress, PracticeExercise } from "./data/types";
...
function ExerciseSection({
  ...
  const handleMarkSolved = useCallback(() => {
    if (!exercise) return;
    const updated = { ...solved, [exercise.id]: !solved[exercise.id] };
    setSolved(updated);
    const progress = getLocalProgress();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {exercises.map((ex, i) => {
          ...
          return (
            ...
            >
              {exSolved && !isActive && <CheckCircle2 className="w-3 h-3" />}
      ...
      <NodeEditor value={showSolution ? exercise.solution : exercise.starterCode} height="220px" />

      <div className="flex items-center gap-2 flex-wrap">
        <button
          type="button"
          onClick={handleMarkSolved}
          className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-md text-[10px] font-mono uppercase tracking-widest transition-colors cursor-pointer ${
            isSolved
              ? "bg-stone-900 dark:bg-stone-50 text-lime-400"
      ...
      {exercise.expectedOutput && (
        <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-white/10 rounded-md overflow-hidden">
          ...
          <pre className="p-4 text-sm text-lime-400 whitespace-pre-wrap font-mono bg-stone-950">
            {exercise.expectedOutput}
          </pre>
        </div>
      )}

      {isSolved && activeIdx < exercises.length - 1 && (
...
export default function NodeLessonDetailPage() {
  ...
  const handleToggleComplete = useCallback(() => {
    ...
    if (newVal && isAuthenticated && sectionSlug) {
      ...
      if (allDone) reportMilestone("COURSE_COMPLETE", "nodejs");
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-8 py-8">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <div className="h-1 w-1 bg-lime-400"></div>
            <span className="text-[10px] font-mono uppercase tracking-widest text-stone-500 dark:text-stone-400">
              nodejs / section {sectionNum} / lesson {lessonNum}
            </span>
          </div>
