...
export default function JsSectionPage() {
  ...
  return (
    <div className="bg-stone-50 dark:bg-stone-950 min-h-[calc(100vh-4rem)]">
      <SEO
        title={`${section.title}, JavaScript Tutorial`}
        description={`Learn ${section.title} in JavaScript. ${section.description}`}
        keywords={`${section.title}, javascript, tutorial`}
        canonicalUrl={canonicalUrl(`/learn/javascript/${sectionSlug}`)}
      />
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-8 py-6 sm:py-8">
        ...
        >
          ...
          <div className="flex items-end justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap text-[10px] font-mono uppercase tracking-widest text-stone-500 dark:text-stone-400">
              <span>
        ...
        >
          {[
            ...
            >
              <div className="w-8 h-8 rounded-md bg-stone-100 dark:bg-white/5 hidden sm:flex items-center justify-center shrink-0">
                <stat.icon className="w-4 h-4 text-lime-600 dark:text-lime-400" />
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-base sm:text-xl font-bold tracking-tight text-stone-900 dark:text-stone-50 truncate">
                  {stat.value}
                </span>
              ...
              return (
                <motion.div key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + i * 0.03 }}>
                  ...
                  >
                    ...
                    <span className={`hidden sm:inline text-[10px] font-mono uppercase tracking-widest shrink-0 ${DIFF_COLOR[lesson.difficulty] || ""}`}>/ {lesson.difficulty.toLowerCase()}</span>
