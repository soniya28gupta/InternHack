...
function ExerciseSection({
  ...
  const handleRun = useCallback(async () => {
    if (!exercise) return;
    const r = await jsEngine.execute(code);
    setResult(r);
  ...
  return (
    ...
    >
      ...
      <JsEditor value={code} onChange={setCode} onRun={handleRun} />
...
export default function JsLessonDetailPage() {
  ...
  const handleToggleComplete = useCallback(() => {
    ...
    if (newVal && isAuthenticated && sectionSlug) {
      ...
      if (allDone) reportMilestone("COURSE_COMPLETE", "javascript");
    }
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-8 py-6 sm:py-8">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <div className="h-1 w-1 bg-lime-400"></div>
            <span className="text-[10px] font-mono uppercase tracking-widest text-stone-500 dark:text-stone-400">
              javascript / section {sectionNum} / lesson {lessonNum}
            </span>
          </div>
