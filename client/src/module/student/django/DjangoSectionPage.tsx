...
function getLocalProgress(): DjangoProgress {
  try {
    return JSON.parse(localStorage.getItem("django-progress") || "{}");
  } catch {
    return {};
  }
}

export default function DjangoSectionPage() {
  const { sectionSlug } = useParams();
  const basePath = "/learn/django";
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated);

  const progress: DjangoProgress = getLocalProgress();
    ...
    return (
      <div className="bg-stone-50 dark:bg-stone-950 min-h-[calc(100vh-4rem)]">
        <div className="max-w-4xl mx-auto px-4 sm:px-8 py-16 text-center">
          ...
          <Link to={basePath} className="text-sm text-lime-600 dark:text-lime-400 hover:underline mt-2 inline-block">
            Return to Django lessons
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-stone-50 dark:bg-stone-950 min-h-[calc(100vh-4rem)]">
      <SEO
        title={`${section.title}, Django Tutorial`}
        description={`Learn ${section.title} in Django. ${section.description}`}
        keywords={`${section.title}, django, tutorial`}
        canonicalUrl={canonicalUrl(`/learn/django/${sectionSlug}`)}
      />
