...
function getLocalProgress(): ReactProgress {
  try {
    return JSON.parse(localStorage.getItem("react-progress") || "{}");
  } catch {
    return {};
  }
}
...
export default function ReactSectionPage() {
  ...
  return (
    <div className="bg-stone-50 dark:bg-stone-950 min-h-[calc(100vh-4rem)]">
      <SEO
        title={`${section.title}, React Tutorial`}
        description={`Learn ${section.title} in react. ${section.description}`}
        keywords={`${section.title}, react, tutorial`}
        canonicalUrl={canonicalUrl(`/learn/react/${sectionSlug}`)}
      />
            ...
            filteredLessons.map((lesson, j) => {
              ...
              return (
                <motion.div key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + j * 0.03 }}>
                  <Link
                    to={`/learn/react/${sectionSlug}/${lesson.id}`}
