...
import type { ReactProgress, PracticeExercise } from "./data/types";
import { reactEngine } from "./lib/react-engine";
import type { ReactRunResult } from "./lib/react-engine";
...
function ExerciseSection({
  ...
  const [result, setResult] = useState<ReactRunResult | null>(null);
  ...
  const handleRun = useCallback(async () => {
    if (!exercise) return;
    const r = await reactEngine.execute(code);
    setResult(r);
  ...
  return (
    ...
    >
      ...
      <ReactEditor value={code} onChange={setCode} onRun={handleRun} />
...
export default function ReactLessonDetailPage() {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-8 py-8">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <div className="h-1 w-1 bg-lime-400"></div>
            <span className="text-[10px] font-mono uppercase tracking-widest text-stone-500 dark:text-stone-400">
              react / section {sectionNum} / lesson {lessonNum}
            </span>
          </div>
