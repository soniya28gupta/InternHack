...
function getLocalProgress(): PythonProgress {
  try {
    return JSON.parse(localStorage.getItem("python-progress") || "{}");
  } catch {
    return {};
  }
}

export default function PythonSectionPage() {
  const { sectionSlug } = useParams();
  const basePath = "/learn/python";
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated);

  const progress: PythonProgress = getLocalProgress();
    ...
    return (
      <div className="bg-stone-50 dark:bg-stone-950 min-h-[calc(100vh-4rem)]">
        <div className="max-w-4xl mx-auto px-4 sm:px-8 py-16 text-center">
          ...
          <Link to={basePath} className="inline-block mt-3 text-[10px] font-mono uppercase tracking-widest text-lime-600 dark:text-lime-400 hover:underline">  / return to python  </Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-stone-50 dark:bg-stone-950 min-h-[calc(100vh-4rem)]">
      <SEO
        title={`${section.title}, Python Tutorial`}
        description={`Learn ${section.title} in python. ${section.description}`}
        keywords={`${section.title}, python, tutorial`}
        canonicalUrl={canonicalUrl(`/learn/python/${sectionSlug}`)}
      />
          ...
          {[
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-xl font-bold tracking-tight text-stone-900 dark:text-stone-50">{stat.value}</span>
                <span className="text-[10px] font-mono uppercase tracking-widest text-stone-500 dark:text-stone-400">/ {stat.label}</span>
              </div>
            </div>
          ))}
        </motion.div>
          ...
          {sectionLessons.map((lesson, i) => {
            ...
            return (
              <motion.div key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + i * 0.03 }}>
                <Link
                  to={`/learn/python/${sectionSlug}/${lesson.id}`}
