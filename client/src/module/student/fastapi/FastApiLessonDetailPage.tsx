...
import type { FastApiProgress, PracticeExercise } from "./data/types";
...
function toggleProgress(lessonId: string): boolean {
  ...
  localStorage.setItem("fastapi-progress", JSON.stringify(progress));
  return !current;
}
...
export default function FastApiLessonDetailPage() {
  ...
  const handleToggleComplete = useCallback(() => {
    ...
    if (newVal && isAuthenticated && sectionSlug) {
      ...
      if (allDone) reportMilestone("COURSE_COMPLETE", "fastapi");
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-8 py-8">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <div className="h-1 w-1 bg-lime-400"></div>
            <span className="text-[10px] font-mono uppercase tracking-widest text-stone-500 dark:text-stone-400">
              fastapi / section {sectionNum} / lesson {lessonNum}
            </span>
          </div>
