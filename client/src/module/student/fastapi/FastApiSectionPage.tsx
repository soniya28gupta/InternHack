    ...
    return (
      <div className="bg-stone-50 dark:bg-stone-950 min-h-[calc(100vh-4rem)]">
        <div className="max-w-4xl mx-auto px-4 sm:px-8 py-16 text-center">
          ...
          <Link to={basePath} className="inline-block mt-3 text-[10px] font-mono uppercase tracking-widest text-lime-600 dark:text-lime-400 hover:underline">  / return to curriculum  </Link>
        </div>
      </div>
    );
  }
          ...
          {sectionLessons.map((lesson, i) => {
            ...
            return (
              <motion.div key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + i * 0.03 }}>
                <Link
                  to={`/learn/fastapi/${sectionSlug}/${lesson.id}`}
