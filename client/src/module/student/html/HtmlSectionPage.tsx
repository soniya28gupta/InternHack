...
export default function HtmlSectionPage() {
  ...
  return (
    <div className="bg-stone-50 dark:bg-stone-950 min-h-[calc(100vh-4rem)]">
      <SEO
        title={`${section.title}, HTML Tutorial`}
        description={`Learn ${section.title} in html. ${section.description}`}
        keywords={`${section.title}, html, tutorial`}
        canonicalUrl={canonicalUrl(`/learn/html/${sectionSlug}`)}
      />
          ...
          {sectionLessons.map((lesson, i) => {
            ...
            return (
              <motion.div key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + i * 0.03 }}>
                <Link
                  to={`/learn/html/${sectionSlug}/${lesson.id}`}
