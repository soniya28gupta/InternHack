...
export default function FlaskSectionPage() {
  const { sectionSlug } = useParams();
  const basePath = "/learn/flask";
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated);

  const progress: FlaskProgress = getLocalProgress();
  ...
  return (
    <div className="bg-stone-50 dark:bg-stone-950 min-h-[calc(100vh-4rem)]">
      <SEO
        title={`${section.title}, Flask Tutorial`}
        description={`Learn ${section.title} in flask. ${section.description}`}
        keywords={`${section.title}, flask, tutorial`}
        canonicalUrl={canonicalUrl(`/learn/flask/${sectionSlug}`)}
      />
          ...
          {sectionLessons.map((lesson, i) => {
            ...
            return (
              <motion.div key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + i * 0.03 }}>
                <Link
                  to={`/learn/flask/${sectionSlug}/${lesson.id}`}
